 ...
 **/
 ...
 def count = 0
 def directoryURL ="x/JIRAAhaVersionSync/"
...
if (productPrefix in productPrefixValue){
	productPrefixFound = true
}

if(excelBody.size()>0  && productPrefixFound == false){
             ...
             String versionName = products[j].replaceAll('�','-')//Replacing the double hyphen
            	if(description[j]== pendingVersion[pendingVersion.size()-k]){
					...
					if(descriptionfound == 0){
							...
							//errorfile.write("")
							errorfile.append("Operation: Not Processed - Same version name exist for different Description ID\n"+"Description: "+description[j]+"\n"+"Current Name: "+"'"+versionName+"'"+"\n\n")//Adding line to the log file for failed version
